...
function App() {
  ...
  console.log('ready', readyToConnect);
  ...
  const connectWallet = () => {
    if (hashconnectService?.availableExtensions) {
  ...
  const disconnectWallet = () => {
    hashconnectService?.clearPairings();
  };
  ...
  useEffect(() => {
    const initHashConnect = async () => {
      ...
      setHashconnectService(hashconnectService);
    };
    const sdk = new SDK();
    setSdk(sdk);
    initHashConnect();
  ...
  const createFile = async () => {
    const signingAcct = hashconnectService?.saveData.pairedAccounts[0];
    ...
    let transactionBytes: Uint8Array | undefined = await hashconnectService?.makeBytes(
      trans,
      signingAcct as string,
    );
  ...
  const executeContract = async () => {
    ...
    const trans = await new ContractExecuteTransaction()
      ...
      .setFunction(
        'set_message',
        new ContractFunctionParameters().addString('Nice to meet you new2222'),
      );
